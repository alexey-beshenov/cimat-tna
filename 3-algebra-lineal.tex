...
Para más detalles, véase \S\ref{sec:campos-linealmente-disjuntos}.
...
\begin{teorema}
  \label{thm:descomposicion-en-campos-ciclotomicos}
...
\pdfbookmark{Clase 14 (28/09/20)}{clase-14}
\subsection{Operaciones con ideales}
\marginpar{\small Clase 14 \\ 28/09/20}

Para un campo de números $K$ el programa puede hacer cálculos con $\O_K$-ideales
fraccionarios.

Un ideal principal $\alpha \O_K$ puede ser especificado por su generador
...
$I$ (y la base fija de $\O_K$).

\begin{proposicion-definicion}
  Sea $H$ una matriz de $n\times n$ con coeficientes en $\ZZ$. Digamos que el
  \textbf{coeficiente mayor} de cada fila es el primer coeficiente no nulo.
  Se dice que $H$ está en la \textbf{forma normal de Hermite} si se cumplen las
  siguientes condiciones.

  \begin{itemize}
  \item[1)] $H$ es una matriz triangular superior con elementos $\ge 0$.

  \item[2)] El coeficiente mayor de cada fila está a la derecha del coeficiente
    mayor de la fila anterior.

  \item[3)] Los elementos arriba del coeficiente mayor son estrictamente menor.

  \item[4)] Los elementos abajo del coeficiente mayor son nulos.
  \end{itemize}

  Para toda matriz $A \in M_{n\times n} (\ZZ)$ existe única $U \in \GL_n (\ZZ)$
  tal que $H = UA$ está en la forma normal de Hermite. En este caso se dice que
  $H$ es la \textbf{forma normal de Hermite} de $A$.

  \begin{proof}
    \cite[\S 2.4.2]{Cohen-GTM138}.
  \end{proof}
...
Para convertir un ideal en la forma normal de Hermite se puede usar la función
\texttt{idealhnf($K$,$I$)}.
...
Tenemos las siguientes operaciones básicas con ideales (todas las funciones
relevantes empiezan por «\texttt{ideal}»):
\begin{itemize}
\item \texttt{idealadd($K$,$\mathfrak{a}$,$\mathfrak{b}$)} --- suma
  $\mathfrak{a}+\mathfrak{b}$,
  \item \texttt{idealmul($K$,$\mathfrak{a}$,$\mathfrak{b}$)} --- producto
    $\mathfrak{ab}$,
  \item \texttt{idealpow($K$,$\mathfrak{a}$,$n$)} --- $\mathfrak{a}^n$,
  \item \texttt{idealinv($K$,$\mathfrak{a}$)} --- $\mathfrak{a}^{-1}$,
  \item \texttt{idealintersect($K$,$\mathfrak{a}$,$\mathfrak{b}$)} ---
    $\mathfrak{a} \cap \mathfrak{b}$.
\end{itemize}
Otras funciones útiles:
\begin{itemize}
\item \texttt{idealdown($K$,$\mathfrak{a}$)} --- el $\ZZ$-ideal
  $\mathfrak{a} \cap \QQ$,
\item \texttt{idealnorm($K$,$\mathfrak{a}$)} --- la norma
  $N_{K/\QQ} (\mathfrak{a})$,
  \item \texttt{idealismaximal($K$,$\mathfrak{a}$)} --- verifica si el ideal es
    maximal.
\end{itemize}
...
Aquí \texttt{idealismaximal($K$,$\mathfrak{a}$)} devuelve \texttt{0} si el ideal
no es maximal, y en el caso contrario, devuelve varias cosas precalculadas, cuyo
...
\texttt{idealtwoelt($K$,$\mathfrak{a}$)}.
...
Interpretación de la salida: $\mathfrak{a} = (3, 1 + \sqrt[3]{2})$.
Por supuesto, es muy interesante ver si algún ideal es principal y admite un
solo generador.  Esto equivale a ver si $\mathfrak{a}$ es trivial en el grupo de
clases $\Cl (K)$, y es más complicado computacionalmente. Veremos cómo hacerlo
...
Para factorizar un ideal $\mathfrak{a}$ en ideales primos, se usa
\texttt{idealfactor($K$,$\mathfrak{a}$)}. Nos va a interesar principalmente
factorización de primos racionales $p \in \ZZ$ en $\O_K$, y para esto sirve la
función \texttt{idealprimedec($K$,$p$)}.
...
\epigraph{The attitude adopted in this book is that while we expect to get
  numbers out of the machine, we also expect to take action based on them, and,
  therefore we need to understand thoroughly what numbers may, or may not,
  mean. To cite the author's favorite motto,

  “The purpose of computing is insight, not numbers,” although some people
  claim,

  “The purpose of computing numbers is not yet in sight.”

  There is an innate risk in computing because “to compute is to sample, and one
  then enters the domain of statistics with all its uncertainties.”}
 {Richard W. Hamming,\\\emph{Introduction to applied numerical analysis}, 1971}
...
Usando la función \texttt{ideallist}, podemos contar los ideales enteros en
$\O_K$ de norma $\le N$ y ver cómo este número depende de $N$. Por ejemplo,
para $K = \QQ (i)$ el resultado se ve de la siguiente manera.

\begin{center}
  \includegraphics[width=15cm]{pic/ideal-count-Qi.pdf}
\end{center}
...
\begin{figure}
  \begin{center}
    \includegraphics[width=12cm]{pic/ideal-count-zeta-5.pdf}

    $K = \QQ (\zeta_5)$
  \end{center}

  \begin{center}
    \includegraphics[width=12cm]{pic/ideal-count-x2-10.pdf}

    $K = \QQ (\sqrt{10})$
  \end{center}

  \begin{center}
    \includegraphics[width=12cm]{pic/ideal-count-x3-19.pdf}
...
Otro fenómeno curioso que podemos investigar es la estadística de
...
Se ve que los $f_i$ deben ser iguales, pero esto no es muy sorprendente:
...
\begin{center}
  ...
  \begin{tabular}{lx{3cm}c}
    $p~(7)$ & \text{factorización} & $f$ \tabularnewline
    \hline
...
es el contenido del teorema de Dirichlet sobre primos en progresiones
aritméticas (véase el apéndice \ref{ap:Dirichlet}).
