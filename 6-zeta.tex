...
\[ \Phi\colon K^\times \hookrightarrow K_\RR^\times, \quad
   \alpha \mapsto (\sigma (\alpha))_\sigma, \]
y el encaje logarítmico
\[ \log |\cdot|\colon K_\RR^\times \to \RR^{r_1+r_2}, \quad
(x_\sigma) \mapsto (\log |\sigma (\alpha)|)_\sigma, \]
donde para evitar repeticiones, entre $\sigma_{r_1+i}$ y
$\overline{\sigma_{r_1 + i}}$ se toma solo un encaje (la conjugación no afecta
el valor absoluto). Por este motivo la imagen está en $\RR^{r_1+r_2}$.

Consideramos la aplicación lineal
\[ \phi\colon \RR^{r_1+r_2} \to \RR, \quad
   x \mapsto \sum_i n_i x_i, \]
donde
\[ n_i = \begin{cases}
  1, & \text{si }1 \le i \le r_1,\\
  2, & \text{si }r_1+1 \le i \le r_1 + r_2.
\end{cases} \]
...
Hemos probado que la aplicación $L$ realiza $\O_K^\times$ como un retículo de
rango completo $r = r_1 + r_2 - 1$ en el subespacio $H = \ker\phi$.
Consideremos unidades fundamentales $u_1, \ldots, u_r \in \O_K^\times$. Entonces
los vectores $L (u_1), \ldots, L (u_r)$ forman una base de $H$. Por otra parte,
el vector $\ell = (1,\ldots,1)$ no está en $H$, así que los $L (u_i)$ junto con
$\ell$ forman una base de $\RR^{r_1+r_2}$: todo $x \in \RR^{r_1+r_2}$ puede ser
expresado como
\begin{equation}
  \label{eqn:base-de-Rr1+r2}
  \lambda_1 \, L (u_1) + \cdots + \lambda_r \, L (u_r) + \lambda \ell
\end{equation}
...
\begin{teorema}
  ...
  \begin{enumerate}
  \item[a)] Para todo $x \in X$ en la expresión de $\log |x|$ en la base
...
\item[a)] Para todo $x \in X$ en la expresión
  $$\log |x| = \lambda_1 \, L (u_1) + \cdots + \lambda_r \, L (u_r) + \lambda \ell$$
...
tenemos
$$\log |\lambda x| = \log|\lambda|\,\ell + \log |x|,$$
...
Primero para la existencia, escribamos $\log |y|$ en términos de nuestra base
de $\RR^{r_1+r_2}$:
$$\log |y| = \lambda_1 \, L (u_1) + \cdots + \lambda_r \, L (u_r) + \lambda \ell.$$
...
\[ \log |z| = L (v^{-1}) + \log |y| =
   ...
   + \lambda_1 \, L (u_1) + \cdots + \lambda_r \, L (u_r) + \lambda \ell =
   \mu_1 \, L (u_1) + \cdots + \mu_r \, L (u_r) + \lambda \ell. \]
...
\begin{ejemplo}
  ...
  **TODO** draw pictures for $X$ and $T$.
\end{ejemplo}
