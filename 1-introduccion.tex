...
La función \texttt{polcylo($n$)} devuelve el $n$-ésimo polinomio ciclotómico.
...
Otro ejemplo: para un primo impar $p$ consideremos las expresiones
$$g = \sum_{1 \le k \le p-1} \legendre{k}{p}\,\zeta_p^k \in \ZZ [\zeta_p],$$
donde $\legendre{k}{p}$ denota el símbolo de Legendre.

\begin{shaded}
\begin{verbatim}
? charsum (p) = sum (k=1,p-1,kronecker(k,p)*Mod (x^k,polcyclo(p)));
? forprime (p=3,23, print ([p, lift(charsum(p)^2)]))
[3, -3]
[5, 5]
[7, -7]
[11, -11]
...
Así de manera experimental descubrimos que para $p$ impar se cumple
$$g^2 = (-1)^{\frac{p-1}{2}}\,p.$$
De hecho, Gauss demostró que
$$g = \begin{cases}
  \sqrt{p}, & \text{si }p \equiv 1 \pmod{4},\\
  i\sqrt{p}, & \text{si }p \equiv 3 \pmod{4}.
\end{cases}.$$
Véase \cite[Chapter~6]{Ireland-Rosen} para más detalles.
...
$$\text{dominio euclidiano} \Longrightarrow \text{dominio de ideales principales} \Longrightarrow \text{dominio de factorización única}.$$
...
\begin{proof}
  ...
  En fin, si $p \equiv 1 \pmod{4}$, entonces $\legendre{-1}{p} = +1$, lo que
  significa que existe un entero $a$ tal que $a^2 \equiv -1 \pmod{p}$.
  Ahora $p \mid (a^2 + 1) = (a + i)\,(a - i)$. Dado que $p \nmid (a \pm i)$,
  esto implica que $p$ no es primo en $\ZZ [i]$. Entonces, $p = \pi\rho$ para
  algunos elementos no-invertibles $\pi$ y $\rho$.
  Ahora $p^2 = N (\pi) \, N (\rho)$ implica que
  $N (\pi) = \pi\,\overline{\pi} = p$. Por el lema de arriba $\pi$ es primo,
  y es fácil ver que $\pi$ y $\overline{\pi}$ no son asociados.
\end{proof}
...
\begin{proposicion}
  ...
  \begin{proof}
    ...
    cuadrática que $\legendre{-3}{p} = +1$. Esto significa que existe un entero
    $a$ tal que $a^2 \equiv -3 \pmod{p}$, y luego se tiene
...
Los anillos de números sirven para formular varias leyes de reciprocidad
parecidas a la reciprocidad cuadrática
$$\legendre{p}{q} = (-1)^{\frac{p-1}{2}\,\frac{q-1}{2}}\,\legendre{q}{p}.$$
Como un ejemplo particular, en la presente sección me gustaría presentar la ley
de reciprocidad cúbica que puede ser formulada en términos de los enteros de
Eisenstein $\ZZ [\zeta_3]$.
...
\[ \legendre{-}{\pi}_3\colon (\ZZ [\zeta_3]/(\pi))^\times
...
\begin{teorema}[Reciprocidad cúbica]
  ...
  \begin{proof}
    Para una prueba con sumas de Gauss véase por ejemplo
    \cite[Chapter~9]{Ireland-Rosen}. El argumento no es muy difícil, pero nos
    llevaría un poco lejos del tema principal del curso.
  \end{proof}
\end{teorema}
...
\begin{ejemplo}
  ...
  \begin{align*}
    4^3 & \equiv 2 \pmod{31},\\
    20^3 & \equiv 2 \pmod{43},\\
    57^3 & \equiv 2 \pmod{109},\\
    32^3 & \equiv 2 \pmod{127},\\
    62^3 & \equiv 2 \pmod{157}. \qedhere
  \end{align*}
\end{ejemplo}
...
$\gcd (x,y,z) = 1$ (lo que también equivale a $\gcd (x,y) = 1$).

Las ternas primitivas corresponden a los puntos \emph{racionales} en el círculo
unitario $x^2 + y^2 = 1$, como por ejemplo
...
\begin{proof}
  ...
  entonces $\gcd (x + yi, \, x - yi) = 1$ (ejercicio). Usando esto y el hecho de
  que su producto es un cuadrado,
...
\begin{comentario}
  La última demostración usa de manera esencial la factorización única en
  $\ZZ [i]$. A saber, usamos que si $\alpha\beta = \gamma^2$ para
  $\gcd (\alpha,\beta) = 1$, entonces $\alpha \sim \alpha'^2$ y
  $\beta \sim \beta'^2$ para algunos $\alpha', \beta'$.

  Consideremos por ejemplo el anillo $\ZZ [\sqrt{-5}]$. En este caso las
  unidades son $\ZZ [\sqrt{-5}]^\times = \{ \pm 1 \}$. Los números
...
demostración con los enteros de Eisenstein $\ZZ [\zeta_3]$. A saber, usando las
terceras raíces de la unidad, la ecuación de Fermat se factoriza como
...
\begin{comentario}
  ...
  el caso general de $n \ge 3$ es un problema abierto.
\end{comentario}
...
\begin{ejercicio}
  ...
  para cualquier DFU $R$, el anillo de polinomios es también un DFU.)
\end{ejercicio}
...
\begin{ejercicio}
  Demuestre que $u = 1 + \zeta_5$ es una unidad en el anillo $\ZZ [\zeta_5]$
  y el subgrupo de $\ZZ [\zeta_5]^\times$ generado por $u$ es infinito.
  (En realidad, $\ZZ [\zeta_5] \cong \mu_{10} (\CC) \times \langle u\rangle$.)
\end{ejercicio}
